...
class AuthController extends Controller
{
    ...
    public function loginAction()
    {
        ...
        if (isset($_POST['btn_login'])) {
            $db = new DBHelper();
            $cred = $_POST['_login'];
            if (isset($_COOKIE[$config['cookie']['login_attempt']])) {
                $va_la = $_COOKIE[$config['cookie']['login_attempt']];
            } else {
                $va_la = 0;
            }
        
            setcookie($config['cookie']['login_attempt'], $va_la + 1, time() + 60 * 60 * 24 * 7);
            ...
            if (password_verify($cred['password'], $row['password'])) {
                ...
                if (isset($_COOKIE[$config['cookie']['login_attempt']]) && $_COOKIE[$config['cookie']['login_attempt']] >= 5) {
                    // lock account
                    $db->update('user', [
                        'locked' => 1,
                    ], [
            ...
            TemplateHelper::renderHTML('/auth/login.tpl.php');
        }
    }
    ...
    public function resettingRequestAction()
    {
        ...
        if (isset($_POST['btn_reset_request'])) {
            ...
            if ($db->has('user', [
                ...
                $body = file_get_contents(TEMPLATEDIR . '/email/reset_request.tpl.html');
                ...
                Helper::sendMail(
                    ...
                    $config['mailer']['replyto_address'],
                    $config['mailer']['replyto_fullname']
                );
            ...
            TemplateHelper::renderHTML('/auth/reset_request.tpl.php');
        }
    }
    
    /**
     * @Route("/resetting/reset", name="auth_resetting_reset")
     * @Method(["GET", "POST"])
     */
    public function resettingResetAction()
    {
        global $config;
        $db = new DBHelper();
        ...
        if (isset($_POST['btn_reset_reset'])) {
            ...
            TemplateHelper::renderHTML('/auth/reset_reset.tpl.php', [
                'token' => $token,
            ]);
        }
    }
